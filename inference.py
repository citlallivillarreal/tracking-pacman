...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance, pacmanPosition, ghostPosition, jailPosition):
        ...
        distpacg = manhattanDistance(pacmanPosition, ghostPosition)
        ...
        if noisyDistance is not None:
            if ghostPosition == jailPosition:
                ...
                return busters.getObservationProbability(noisyDistance, distpacg)
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            ...
            for pos in self.allPositions:
                newbelief[pos] = newbelief[pos] + newPosDist[pos] * self.beliefs[oldPos]
        self.beliefs = newbelief.copy()
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for part in self.particles:
            weight_dis[part] = self.getObservationProb(observation, pacpos, part, jailpos)*beliefs[part]
        if weight_dis.total() == 0:
            self.initializeUniformly(gameState)
        else:
            self.particles = [weight_dis.sample() for _ in range(self.numParticles)]
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = [random.choice(positions) for _ in range(self.numParticles)]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostpos in self.particles:
            for i in range(self.numGhosts):
                ...
                newobservation = newobservation * self.getObservationProb(observation[i], pacpos, ghostpos[i], jailpos)
            weight_dist[ghostpos] = newobservation*beliefs[ghostpos]
            newobservation = 1
